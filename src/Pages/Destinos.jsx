            ...
            <div className="card bg-input-bg shadow-md rounded-3xl p-6 w-full max-w-lg">
                ...
                <div className="input-container flex flex-col items-center justify-center space-y-4">
                    <input
                        ...
                        className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent-primary"
                    />
                    <div id="destinos-suggestions" className="w-full space-y-2">
                        {filteredDestinos.length > 0 ? (
                            filteredDestinos.map((destino, index) => (
                                <div key={index} className="destinos-suggestion-item flex justify-between p-2 bg-accent-muted rounded-lg shadow">
                                    <span className="text-text-primary">{destino.DESTINO}</span>
                                    <span className="text-accent-primary font-semibold">S/. {destino.COSTO_DE_ENVIO}</span>
                                </div>
                            ))
                        ) : (
                            searchTerm && (
